...
class Experts extends REST_Controller
{
	...
	public function sme_profile_get(){
		...
		log_message('debug', 'Class: Experts - sme_profile_get() called');
		if (empty($this->current_user)) {
			log_message('debug', 'Class: Experts - sme_profile_get() there is no user details');
			$this->set_response(null, REST_Controller::HTTP_FORBIDDEN);
		} else {
			$sme_id = $this->get("sme_id");
			$response = $this->Expert->getprofile2($sme_id);
			if(!empty($response)){
				log_message('debug', 'Class: Experts - sme_profile_get() $response');
				$this->set_response($response, REST_Controller::HTTP_OK);
			}else{
				log_message('debug', 'Class: Experts - sme_profile_get() there is no sme user detail.');
				$this->set_response([
						'status'=> false,
		...
		log_message('debug', 'Class: Experts - sme_profile_get(). Total execution time: ' . $time_elapsed_secs);
	}
}