		...
		<div <?php echo $layout_type ?> class="wpb_wrapper <?php echo esc_attr($css_class) ?>">
			...
			<?php foreach ($images as $id => $attach_id):
				...
				if ($attach_id > 0) {
					...
					$new_img_size =  is_array($img_size) && isset($img_size[$id]) ? trim($img_size[$id]) : $img_size;
					$post_thumbnail['thumbnail'] = REVIJA_HELPER::get_the_thumbnail($attach_id, $new_img_size, $thumbnail_atts);
					$post_thumbnail['p_img_large'] = REVIJA_HELPER::get_post_attachment_image($attach_id, '');
				} else {
					$post_thumbnail = array();
