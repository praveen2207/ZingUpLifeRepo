...
class WPBakeryShortCode_VC_mad_pricing_box extends WPBakeryShortCode {
	...
	protected function content($atts, $content = null) {
		...
		$this->atts = shortcode_atts(array(
			...
			'add_hot' => 'off',
			'box_style' => 'bg_color_orange',
			'header_bg_color' => '#3e454c',
			'main_bg_color' => '#ff680d',
			'link' => '',
			'css_animation' => ''
	...
	public function html() {
		...
		$output = $el_class = $hot_class = $link = $add_hot = $css_animation = $title = $currency = $price = $time = $features = $add_badge = $box_style = $features = $header_bg_color = $main_bg_color = '';
		...
		if ($add_hot == 'on') {
			$hot_class = $this->getExtraClass('active');
		}

		$css_class = apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $box_style . $hot_class . $animations, $this->settings['base']);
		...
		$output .= "<div class='pricing-box {$el_class}'>";
			$output .= "<div class='p_table {$css_class}'>";
				if ($add_hot == 'on') {
					$output .= '<span class="hot_p"><img src="'. get_template_directory_uri() .'/images/best.png" alt=""></span>';
				}
				$output .= $title;
				$output .= "<div {$main_bg_color} class='price'>";
					$output .= '<dl>';
						$output .= $price;
						$output .= $time;
					$output .= '</dl>';
