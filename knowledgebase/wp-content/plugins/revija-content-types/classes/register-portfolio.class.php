	...
	class MAD_PORTFOLIO extends MAD_CONTENT_TYPES {
		...
		public function init() {
			$args = array(
				...
				'hierarchical' => true,
				'menu_position' => null,
				'supports' => array('title', 'editor', 'thumbnail', 'excerpt', 'tags', 'comments', 'custom-fields'),
			...
			register_taxonomy("portfolio_label", array($this->slug), array(
				"hierarchical" => true,
				"labels" => $this->getTaxonomyLabels(
					__('Label', 'mad_app_textdomain'),
					__('Label', 'mad_app_textdomain')
				),
				"singular_label" => __("portfolio_label", 'mad_app_textdomain'),
				"show_tagcloud" => true,
				'query_var' => true,
				'rewrite' => true,
			...
			if (is_admin() && class_exists('Tax_Meta_Class')){
				  ...
				  $config = array(
					...
					'pages' => array('portfolio_label'),        // taxonomy name, accept categories, post_tag and custom taxonomies
		...
		public function show_edit_columns_content($column) {
			...
			switch ($column) {
				...
				case "portfolio_entries":
					...
					break;
				case "format":
					$output = get_post_meta($post->ID, 'mad_format_icon', true);
					echo $output;
					break;
			}
		}
