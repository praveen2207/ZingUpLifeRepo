...
switch ( $source ) {
	...
	case 'featured_image':
		...
		$img = wpb_getImageBySize( array(
			'attach_id' => $img_id,
			'thumb_size' => $img_size,
			'class' => 'vc_single_image-img scale_image',
		) );
	...
	case 'external_link':
		...
		$img = array(
			'thumbnail' => '<img class="vc_single_image-img scale_image" ' . $hwstring . ' src="' . $custom_src . '" />',
		);
		break;
...
if ( ! $img ) {
	$img['thumbnail'] = '<img class="vc_img-placeholder vc_single_image-img scale_image" src="' . $default_src . '" />';
}
...
$output = '
	<div class="' . esc_attr( trim( $css_class ) ) . '">
		...
		<figure class="wpb_wrapper vc_figure"><div class="scale_image_container">
			' . $html . '
		</div></figure>
	</div>
';
