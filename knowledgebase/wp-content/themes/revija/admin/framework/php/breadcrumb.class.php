...
if (!function_exists('mad_breadcrumbs')) {
	...
	function mad_breadcrumbs( $args = array() ) {
		...
		$defaults = array(
			...
			'separator' => '&raquo;',
			'front_page' => true,
			'show_home' => esc_html__( 'Home', 'revija' ),
		...
		if (is_front_page()) {
			...
			if (is_tax() || is_category() || is_tag()) {
				...
				if (get_query_var( 'minute' ) && get_query_var('hour')) {
					$trail['end'] = get_the_time( esc_html__('g:i a', 'revija' ));
				} elseif ( get_query_var( 'minute' ) ) {
					$trail['end'] = sprintf( esc_html__('Minute %1$s', 'revija' ), get_the_time( esc_html__( 'i', 'revija' ) ) );
				} elseif ( get_query_var( 'hour' ) ) {
					$trail['end'] = get_the_time( esc_html__( 'g a', 'revija'));
				}
			} elseif (is_date()) {
				...
				if (is_day()) {
					$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'revija' ) ) . '">' . get_the_time( esc_html__( 'Y', 'revija' ) ) . '</a>';
					$trail[] = '<a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" title="' . get_the_time( esc_attr__( 'F', 'revija' ) ) . '">' . get_the_time( esc_html__( 'F', 'revija' ) ) . '</a>';
					$trail['end'] = get_the_time( esc_html__( 'j', 'revija' ) );
				} elseif ( get_query_var( 'w' ) ) {
					$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'revija' ) ) . '">' . get_the_time( esc_html__( 'Y', 'revija' ) ) . '</a>';
					$trail['end'] = sprintf( esc_html__( 'Week %1$s', 'revija' ), get_the_time( esc_attr__( 'W', 'revija' ) ) );
				} elseif ( is_month() ) {
					$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'revija' ) ) . '">' . get_the_time( esc_html__( 'Y', 'revija' ) ) . '</a>';
					$trail['end'] = get_the_time( esc_html__( 'F', 'revija' ) );
				} elseif ( is_year() ) {
					$trail['end'] = get_the_time( esc_html__( 'Y', 'revija' ) );
				}
			}
		} elseif ( is_search() ) {
			$trail['end'] = sprintf( esc_html__( 'Search results for &quot;%1$s&quot;', 'revija' ), esc_attr( get_search_query() ) );
		} elseif ( is_404() ) {
			$trail['end'] = esc_html__( '404 Not Found', 'revija' );
		}

		if (is_array($trail)) {
			if (!empty($trail['end'])) {
				if (!is_search()) {
					$trail['end'] = mad_post_content_truncate($trail['end'], $truncate, " ", $pad = "...");
				}
