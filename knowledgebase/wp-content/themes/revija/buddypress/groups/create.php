	...
	<form action="<?php bp_group_creation_form_action(); ?>" method="post" id="create-group-form" class="standard-form" enctype="multipart/form-data">
		...
		<div class="item-body" id="group-create-body">
			...
			<?php if ( bp_is_group_creation_step( 'group-details' ) ) : ?>
				...
				<div>
					<label for="group-name"><?php esc_html_e( 'Group Name', 'buddypress' ); ?><span class="required">*</span></label>
					<input type="text" name="group-name" id="group-name" aria-required="true" value="<?php bp_new_group_name(); ?>" />
				</div>

				<div>
					<label for="group-desc"><?php esc_html_e( 'Group Description', 'buddypress' ); ?><span class="required">*</span></label>
			...
			<?php if ( bp_is_group_creation_step( 'group-settings' ) ) : ?>
				...
				<h3 class="section_title"><?php esc_html_e( 'Privacy Options', 'buddypress' ); ?></h3>
				
				<div class="radio">
					
					<input id="group-status-public" type="radio" name="group-status" value="public"<?php if ( 'public' == bp_get_new_group_status() || !bp_get_new_group_status() ) { ?> checked="checked"<?php } ?> /> 
					<label for="group-status-public" ><strong><?php esc_html_e( 'This is a public group', 'buddypress' ); ?></strong></label>
					<ul>
					...
					<input id="group-status-private" type="radio" name="group-status" value="private"<?php if ( 'private' == bp_get_new_group_status() ) { ?> checked="checked"<?php } ?> />
					<label for="group-status-private" >	<strong><?php esc_html_e( 'This is a private group', 'buddypress' ); ?></strong>
					</label>
					<ul>
					...
					<input id="group-status-hidden" type="radio" name="group-status" value="hidden"<?php if ( 'hidden' == bp_get_new_group_status() ) { ?> checked="checked"<?php } ?> />
					<label for="group-status-hidden" >	<strong><?php esc_html_e('This is a hidden group', 'buddypress' ); ?></strong>
					</label>
					<ul>
				...
				<h3 class="section_title"><?php esc_html_e( 'Group Invitations', 'buddypress' ); ?></h3>
				...
				<div class="radio">
					...
					<input id="group-invite-status-members"  type="radio" name="group-invite-status" value="members"<?php bp_group_show_invite_status_setting( 'members' ); ?> />
					<label for="group-invite-status-members" >	<strong><?php esc_html_e( 'All group members', 'buddypress' ); ?></strong>
					</label>

					
					<input  id="group-invite-status-mods" type="radio" name="group-invite-status" value="mods"<?php bp_group_show_invite_status_setting( 'mods' ); ?> />
					<label for="group-invite-status-mods" ><strong><?php esc_html_e( 'Group admins and mods only', 'buddypress' ); ?></strong>
					</label>

					
					<input  id="group-invite-status-admins" type="radio" name="group-invite-status" value="admins"<?php bp_group_show_invite_status_setting( 'admins' ); ?> />
					<label for="group-invite-status-admins" >	<strong><?php esc_html_e( 'Group admins only', 'buddypress' ); ?></strong>
					</label>
				</div>
				...
				<?php if ( bp_is_active( 'forums' ) ) : ?>
					...
					<h3 class="section_title"><?php esc_html_e( 'Group Forums', 'buddypress' ); ?></h3>
					...
					<?php if ( bp_forums_is_installed_correctly() ) : ?>
						...
						<div class="checkbox">
						    ...
						    <label><?php esc_html_e( 'Enable discussion forum', 'buddypress' ); ?></label>
						</div>
						
						
					<?php elseif ( is_super_admin() ) : ?>

						<p><?php printf( esc_html__( '<strong>Attention Site Admin:</strong> Group forums require the <a href="%s">correct setup and configuration</a> of a bbPress installation.', 'buddypress' ), bp_core_do_network_admin() ? network_admin_url( 'settings.php?page=bb-forums-setup' ) :  admin_url( 'admin.php?page=bb-forums-setup' ) ); ?></p>
			...
			<?php if ( 'crop-image' != bp_get_avatar_admin_step() ) : ?>
				...
				<div class="submit" id="previous-next">
					...
					<?php if ( !bp_is_first_group_creation_step() ) : ?>
						...
						<input class="button button_grey" type="button" value="<?php esc_attr_e( 'Back to Previous Step', 'buddypress' ); ?>" id="group-creation-previous" name="previous" onclick="location.href='<?php bp_group_creation_previous_link(); ?>'" />
					...
					<?php if ( !bp_is_last_group_creation_step() && !bp_is_first_group_creation_step() ) : ?>
						...
						<input class="button button_grey"  type="submit" value="<?php esc_attr_e( 'Next Step', 'buddypress' ); ?>" id="group-creation-next" name="save" />
					...
					<?php if ( bp_is_first_group_creation_step() ) : ?>
						...
						<input class="button button_grey"  type="submit" value="<?php esc_attr_e( 'Create Group and Continue', 'buddypress' ); ?>" id="group-creation-create" class="button button_grey" name="save" />
					...
					<?php if ( bp_is_last_group_creation_step() ) : ?>
						...
						<input class="button button_grey"  type="submit" value="<?php esc_attr_e( 'Finish', 'buddypress' ); ?>" id="group-creation-finish" name="save" />
