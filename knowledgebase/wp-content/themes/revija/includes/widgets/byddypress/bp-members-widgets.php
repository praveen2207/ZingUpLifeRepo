 ...
 */
class Mad_BP_Core_Members_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		$description = esc_html__( 'A dynamic list of recently active, popular, and newest members', 'buddypress' );
	...
	public function widget( $args, $instance ) {
		...
		<?php if ( bp_has_members( $members_args ) ) : ?>
			...
			<div class="item-options clearfix" id="members-list-options">
				<a href="<?php bp_members_directory_permalink(); ?>" id="newest-members" <?php if ( 'newest' === $settings['member_default'] ) : ?>class="selected"<?php endif; ?>><h5><?php esc_html_e( 'Newest', 'buddypress' ); ?></h5></a>
				<a href="<?php bp_members_directory_permalink(); ?>" id="recently-active-members" <?php if ( 'active' === $settings['member_default'] ) : ?>class="selected"<?php endif; ?>><h5><?php esc_html_e( 'Active', 'buddypress' ); ?></h5></a>

				<?php if ( bp_is_active( 'friends' ) ) : ?>

				<a href="<?php bp_members_directory_permalink(); ?>" id="popular-members" <?php if ( 'popular' === $settings['member_default'] ) : ?>class="selected"<?php endif; ?>><h5><?php esc_html_e( 'Popular', 'buddypress' ); ?></h5></a>
			...
			<ul id="members-list" class="item-list">
				...
				<?php while ( bp_members() ) : bp_the_member(); ?>
					...
					<li class="vcard clearfix">
						<div class="item-avatar scale_image_container">
							<a href="<?php bp_member_permalink() ?>" title="<?php bp_member_name(); ?>"><?php bp_member_avatar('type=thumb&width=80&height=80'); ?></a>
						</div>

						<div class="item post_text">
							<div class="item-title fn"><a href="<?php bp_member_permalink(); ?>" title="<?php bp_member_name(); ?>"><h4><?php bp_member_name(); ?></h4></a></div>
							<div class="item-meta">
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_name( 'link_title' )) ?>">
				<input type="checkbox" name="<?php echo esc_attr($this->get_field_name( 'link_title' )) ?>" value="1" <?php checked( $link_title ) ?> />
 ...
 * @since BuddyPress (1.0.3)
 */
class Mad_BP_Core_Whos_Online_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		$description = esc_html__( 'Profile photos of online users', 'buddypress' );
		parent::__construct( false, $name, array(
			'description' => $description,
	...
	public function widget( $args, $instance ) {
		...
		<?php if ( bp_has_members( $members_args ) ) : ?>
			...
			<ul class="writers_list var2 clearfix">
				<?php while ( bp_members() ) : bp_the_member(); ?>

				<li>
					<a href="<?php bp_member_permalink(); ?>" title="<?php bp_member_name(); ?>">
					<div class="item-avatar">
						<?php bp_member_avatar('width=80&height=80'); ?>
					</div>
					</a>
				</li>	
				<?php endwhile; ?>
	...
	public function parse_settings( $instance = array() ) {
		return bp_parse_args( $instance, array(
			'title' 	     => esc_html__( "Who's Online", 'buddypress' ),
			'max_members' 	 => 15,
		), 'members_widget_settings' );
	}
}
 ...
 * @since BuddyPress (1.0.3)
 */
class Mad_BP_Core_Recently_Active_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		$description = esc_html__( 'Profile photos of recently active members', 'buddypress' );
		parent::__construct( false, $name, array(
			'description' => $description,
	...
	public function widget( $args, $instance ) {
		...
		<?php if ( bp_has_members( $members_args ) ) : ?>
			<ul class="writers_list var2 clearfix" style= "margin-left: 0;">
				...
				<a href="<?php bp_member_permalink(); ?>" title="<?php bp_member_name(); ?>">
					<div class="item-avatar">
						...
						<?php bp_member_avatar('type=full&width=80&height=80'); ?>
	...
	public function parse_settings( $instance = array() ) {
		return bp_parse_args( $instance, array(
			'title' 	     => esc_html__( 'Recently Active Members', 'buddypress' ),
			'max_members' 	 => 15,
 ...
 */
function bp_core_ajax_widget_members1() {
	...
	if ( bp_has_members( $members_args ) ) : ?>
		...
		<?php while ( bp_members() ) : bp_the_member(); ?>
			<li class="vcard clearfix">
				<div class="item-avatar scale_image_container">
					<a href="<?php bp_member_permalink(); ?>"><?php bp_member_avatar('type=thumb&width=80&height=80'); ?></a>
				</div>
...
remove_action( 'wp_ajax_widget_members',        'bp_core_ajax_widget_members' );
remove_action( 'wp_ajax_nopriv_widget_members', 'bp_core_ajax_widget_members' );

add_action( 'wp_ajax_widget_members',        'bp_core_ajax_widget_members1' );
add_action( 'wp_ajax_nopriv_widget_members', 'bp_core_ajax_widget_members1' );
