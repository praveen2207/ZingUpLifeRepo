...
$("#cs_vendor_packages_treatments_form").on("submit", function(e){	
     var pattern = /^[0-9%]+$/;
...
$("#cs_vendor_business_service_edit_form").on("submit", function(e){
 var pattern = /^[0-9%]+$/;	
	var discount = $(this).find('.service_discount').val();
	var discount_start_date = $(this).find('.service_discount_start_date').val();
	var discount_end_date = $(this).find('.service_discount_end_date').val();
    ...
    jQuery.validator.addMethod("minplength", function (value, element) {
     ...
     $('#cs_vendor_service_slots').validate({
 		      rules: {
    ...
    $('#cs_vendor_one_time_slots').validate({
        rules: {
           service_name:{
                required: true   
        },
...
$('#cs_error_popup_type').hide();
        ...
        $.ajax({
            type: 'POST',
            url: base_url + 'cs/adding_business_programs',
     ...
     $('body').on('click', '.package_delete', function () {
        ...
        if (confirm("Are you sure you want to delete this package/treatment!") == true)
        {
            $.ajax({
                type: 'POST',
                url: base_url + 'cs/delete_business_package',
     ...
     $('body').on('click', '#service_delete', function () {
       ...
       // alert(service_id);
        if (confirm("Are you sure you want to delete this service!") === true)
        {
            ...
            $.ajax({
                type: 'POST',
                url: base_url + 'cs/delete_business_services',
      ...
      $('body').on('click', '#business_gallery_delete', function () {
          ...
          if (confirm("Are you sure you want to delete this image!") === true)
        {
            ...
            $.ajax({
                type: 'POST',
                url: base_url + 'cs/delete_business_gallery',
    ...
    $('body').on('click', '#business_service_gallery_delete', function () {
        ...
        if (confirm("Are you sure you want to delete this image!") === true)
        {
            $.ajax({
                type: 'POST',
                url: base_url + 'cs/business_service_gallery_delete',
                data: {service_id: service_id},
                dataType: "json",
                success: function (data) {
                    // console.log(data);
 ...
 $('.service-name-filter input').on('keyup', function () {
        ...
        $.ajax({
            url: base_url + 'cs/service_search_filter',
            type: 'POST',
                ...
                if (len > 0)
                {
                    ...
                    $.each(data, function (index, value) {
                        result += '<tr>' +
                                ...
                                '<li><a class="blue" href="'+base_url+'cs/business_service_gallery/'+value.id+'">View Gallery</a></li>'+
                                '</ul>'+ 
                                '</td>'+
                                '<td class="blue">'+                                
                                '<ul class="backend-actions">'+
                                '<li><a class="blue" href="'+base_url+'cs/business_service_edit/'+value.id+'">Edit</a></li>'+
                                '<li>|</li>'+
 ...
 $('body').on('click', '#one_time_service_slots_delete', function () {
       ...
       // alert(service_id);
        if (confirm("Are you sure you want to delete this slot!") === true)
        {
            $.ajax({
                type: 'POST',
                url: base_url + 'cs/one_time_service_slots_delete',
 ...
 $('body').on('click', '#membership_delete', function () {
       ...
       // alert(service_id);
        if (confirm("Are you sure you want to delete this membership!") === true)
        {
            $.ajax({
                type: 'POST',
                url: base_url + 'cs/delete_offerings_memberships',
    ...
    $('body').on('click', '#one_day_package_delete', function () {
        ...
        if (confirm("Are you sure you want to delete this package!") === true)
        {
            $.ajax({
                type: 'POST',
                url: base_url + 'cs/delete_one_day_package',
                data: {package_id: package_id},
                dataType: "json",
                success: function (data) {
                    location.reload();
...
$('body').on('change', '.programs', function () {
        ...
        $.ajax({
            type: 'POST',
            url: base_url + 'cs/get_business_services',
