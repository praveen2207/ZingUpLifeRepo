...
<form action="<?php echo trailingslashit( bp_displayed_user_domain() . bp_get_settings_slug() . '/profile' ); ?>" method="post" class="standard-form" id="settings-form">
	...
	<?php if ( bp_xprofile_get_settings_fields() ) : ?>
		...
		<?php while ( bp_profile_groups() ) : bp_the_profile_group(); ?>
			...
			<?php if ( bp_profile_fields() ) : ?>
				...
				<table class="profile-settings table_type_1" id="xprofile-settings-<?php bp_the_profile_group_slug(); ?>">
					<thead>
						<tr>
	...
	<div class="submit">
		<input id="submit" class="button button_grey" type="submit" name="xprofile-settings-submit" value="<?php esc_attr_e( 'Save Settings', 'buddypress' ); ?>" class="auto" />
	</div>
