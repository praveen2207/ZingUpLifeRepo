<div id="buddypress">
	...
	<form action="" method="post" id="forums-directory-form" class="dir-form">
		...
		<div class="item-list-tabs" role="navigation">
			<ul>
				<li class="selected" id="forums-all"><a href="<?php echo trailingslashit( bp_get_root_domain() . '/' . bp_get_forums_root_slug() ); ?>"><?php printf( esc_html__( 'All Topics <span>%s</span>', 'buddypress' ), bp_get_forum_topic_count() ); ?></a></li>

				<?php if ( is_user_logged_in() && bp_get_forum_topic_count_for_user( bp_loggedin_user_id() ) ) : ?>

					<li id="forums-personal"><a href="<?php echo trailingslashit( bp_loggedin_user_domain() . bp_get_forums_slug() . '/topics' ); ?>"><?php printf( esc_html__( 'My Topics <span>%s</span>', 'buddypress' ), bp_get_forum_topic_count_for_user( bp_loggedin_user_id() ) ); ?></a></li>
		...
		<?php if ( is_user_logged_in() ) : ?>
			...
			<?php if ( bp_is_active( 'groups' ) && bp_has_groups( 'user_id=' . bp_loggedin_user_id() . '&type=alphabetical&max=100&per_page=100' ) ) : ?>
				...
				<form action="" method="post" id="forum-topic-form" class="standard-form">
					...
					<a name="post-new"></a>
					<h5><?php esc_html_e( 'Create New Topic:', 'buddypress' ); ?></h5>
					...
					<label><?php esc_html_e( 'Post In Group Forum:', 'buddypress' ); ?></label>
					<select id="topic_group_id" name="topic_group_id">

						<option value=""><?php /* translators: no option picked in select box */ esc_html_e( '----', 'buddypress' ); ?></option>
