...
if (!class_exists('REVIJA_WC_CURRENCY_SWITCHER')) {

	class REVIJA_WC_CURRENCY_SWITCHER extends REVIJA_WOOCOMMERCE_CONFIG {
		...
		function __construct() {
			...
			$this->settings = array(
				array( 'name' => esc_html__( 'Currency Codes', 'revija' ), 'type' => 'title', 'desc' => '', 'id' => 'wc_currency_code' ),
				array(
					'name' => esc_html__('Codes', 'revija'),
					'desc' 		=> '',
				...
				array( 'name' => esc_html__( 'Open Exchange Rate API', 'revija' ), 'type' => 'title', 'desc' => '', 'id' => 'product_enquiry' ),
				array(
					'name' => esc_html__('App Key', 'revija'),
					'desc' 		=> sprintf( esc_html__('(optional) If you have an <a href="%s">Open Exchange Rate API app ID</a>, enter it here.', 'revija'), 'https://openexchangerates.org/signup' ),
		...
		public function enqueue_currency_js() {
			...
			wp_enqueue_script( REVIJA_PREFIX . 'money_js', $assets_js_url . 'money.min.js', array( REVIJA_PREFIX . 'woocommerce-mod' ), '', true );
			wp_enqueue_script( REVIJA_PREFIX . 'accountingjs', $assets_js_url . 'accounting.min.js', array( REVIJA_PREFIX . 'woocommerce-mod' ), '', true );
			wp_enqueue_script( REVIJA_PREFIX . 'converter_js', $assets_js_url . 'converter.js', array( REVIJA_PREFIX . 'woocommerce-mod', REVIJA_PREFIX . 'money_js', REVIJA_PREFIX . 'accountingjs' ), '', true );
			...
			wp_localize_script( REVIJA_PREFIX . 'woocommerce-mod', 'wc_currency_converter_params', array(
				...
				'i18n_oprice'      => esc_html__( 'Original price:', 'revija'),
	...
	new REVIJA_WC_CURRENCY_SWITCHER();
