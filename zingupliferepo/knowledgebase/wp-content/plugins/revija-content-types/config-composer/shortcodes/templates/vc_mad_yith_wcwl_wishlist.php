		...
		<div class="<?php echo $css_class ?>">
			...
			<form id="yith-wcwl-form" action="<?php echo esc_url( $yith_wcwl->get_wishlist_url() ) ?>" method="post">
				...
				<table class="shop_table cart wishlist_table" cellspacing="0">
					...
					<thead>
						<tr>
							<th><?php esc_html_e( 'Product Image',  'revija' ); ?></th>
							<th><?php esc_html_e( 'Product Name & Category', 'revija'); ?></th>
							<th><?php esc_html_e( 'Price', 'revija' ); ?></th>
							<th><?php esc_html_e( 'Quantity', 'revija' ); ?></th>
							<th><?php esc_html_e( 'Action', 'revija' ); ?></th>
						</tr>
					</thead>
					...
					if ( count( $wishlist ) > 0 ) :
						foreach( $wishlist as $values ) :
							...
							if ( $product_obj !== false && $product_obj->exists() ) : ?>
								...
								<tr id="yith-wcwl-row-<?php echo $values['ID'] ?>">
									...
									<td class="product-thumbnail" data-title="<?php esc_html_e( 'Product Image',  'revija' ); ?>">
									...
									<td class="product-name" data-title="<?php esc_html_e( 'Product Name & Category', 'revija'); ?>">
										<h4>
									...
									<td class="product-subtotal" data-title="<?php esc_html_e( 'Price', 'revija' ); ?>">
										<?php
										if( $product_obj->price != '0' ) {
											...
											echo apply_filters( 'yith_free_text', esc_html__( 'Free!', 'revija' ) );
										}
										?>
									</td>

									<td class="product-quantity" data-title="<?php esc_html_e( 'Quantity', 'revija' ); ?>">
										<?php
									...
									<td class="product-action" data-title="<?php esc_html_e( 'Action', 'revija' ); ?>">
										<?php
										...
										<a href="javascript:void(0)" onclick="<?php echo $remove_wishlist ?>" class="remove " title="<?php esc_html_e( 'Remove', 'revija') ?>"><?php esc_html_e( 'Remove', 'revija' ) ?></a>
									</td>
						...
						<tr>
							<td colspan="6" class="wishlist-empty"><?php esc_html_e( 'No products were added to the wishlist', 'revija' ) ?></td>
						</tr>
