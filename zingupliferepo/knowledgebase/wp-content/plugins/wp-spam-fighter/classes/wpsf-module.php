...
if (!class_exists('WPSF_Module')) {
    ...
    abstract class WPSF_Module
    {
        ...
        {
            do_action('wpsf_render_template_pre', $default_template_path, $variables);

            $template_path = locate_template(basename($default_template_path));
            if (!$template_path) {
                $template_path = dirname(__DIR__) . '/views/' . $default_template_path;
            }
            $template_path = apply_filters('wpsf_template_path', $template_path);
            ...
            if (is_file($template_path)) {
                ...
                $template_content = apply_filters('wpsf_template_content', ob_get_clean(), $default_template_path, $template_path, $variables);
            } else {
                $template_content = '';
            }

            do_action('wpsf_render_template_post', $default_template_path, $variables, $template_path, $template_content);
