...
class Curl {
	...
	public function set_defaults()
	{
        ...
        //This function use to post row data for event drop
        public function eventDrop_post($rowBodyStr){
           
            $CI = get_instance();
            $CI->load->model('Event_service_model');
            
            $array=explode(",",$rowBodyStr);
            ...
            $mainUrl  = 'http://ec2-35-166-83-81.us-west-2.compute.amazonaws.com:8080/ZULServices/zul/DropEventService/dropevent';//STAGING 
           // $mainUrl = 'http://54.191.29.141:8080/ZULServices/zul/DropEventService/dropevent'; //Prod END Point
            log_message('debug', 'eventDrop_post called !$mainUrl: '.$mainUrl);
            log_message('debug', 'eventDrop_post called !$rowBodyStr: '.$rowBodyStr);
            $headerOpt  = array('Content-Type: application/json','Content-Length: ' . strlen($rowBodyStr));
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL,            $mainUrl );
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1 );
            curl_setopt($ch, CURLOPT_POST,           1 );
            curl_setopt($ch, CURLOPT_POSTFIELDS,     $rowBodyStr ); 
            curl_setopt($ch, CURLOPT_HTTPHEADER, $headerOpt);
            $result=curl_exec ($ch);
            log_message('debug', 'closing service!$result: '.$result);
            return $result;
        }
